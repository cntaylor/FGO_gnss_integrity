...
1. Store "snapshots" with all the information needed to localize. 
2. Store results of processing the snapshot information by different "methods" so that comparisons can be made in terms of outlier rejection and performance

# Snapshot information
For a complete snapshot, the following information should be available:
...
Note that the goal of this project is to use Monte Carlo simulations to test out snapshots with lots of different outliers and possible problems.  Therefore, each "snapshot" is divided into two portions.  The first is the true location of the receiver and the locations of all the satellites.  The second portion is a Monte Carlo sample of what the pseudoranges will be.  There will be multiple sets of pseudoranges per "snapshot" representing possible errors on the pseduoranges.
...
To enable a setup that records everything listed above, we will use a SQL database with 7 interacting tables.  Note that often there are two or three tables that are really used to store a single piece of information, but by splitting out the tables, data storage will be far more efficient.  At a high level:
  - Tables 1 & 2 store the first portion of the snapshot information for all the snapshots.  This provides sufficient information to generate noiseless pseudo-range estimates
...
1.  This table stores information for each timestep.  Each entry in the table will have a Sanpshot_ID, what dataset it came from, what time is associated with that snapshot, and the true location of the receiver.  Entries are indexed by "Sanpshot_ID".
2.  A table for each satellite.  This will have the Snapshot_ID (from Table 1) + a satellite num (starting at 0 for each SanpshotID), and location of that satellite for that snapshot.
...
4.  A table with unique Monte Carlo ID for each sample (really, a table of all samples ever created).  For each sample, it records which snapshot_ID  and MC_Run_ID's it pertains to.
